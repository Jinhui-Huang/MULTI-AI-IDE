...
export class ExtensionApiClient {
  ...
  async get(path: string): Promise<any> { return this.request('GET', path); }
  async post(path: string, body?: unknown): Promise<any> { return this.request('POST', path, body); }
  async put(path: string, body?: unknown): Promise<any> { return this.request('PUT', path, body); }
