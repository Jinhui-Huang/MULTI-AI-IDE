...
const CODEX_TIMEOUT_MS = Number(process.env.CODEX_TIMEOUT_MS) || 10 * 60 * 1000; // 10 min default
...
const runCodex = (prompt, session, onProgress, { images = [] } = {}) =>
  new Promise((resolve) => {
    ...
    let stderr = '';
    let resolved = false;
    const steps = [];             // accumulated step objects
    const itemMap = new Map();    // item id → index in steps[]
    ...
    // Timeout: kill the process and resolve if it takes too long
    ...
    const handleEvent = (evt) => {
      ...
      if (item.type === 'agent_message') {
        ...
        if (evt.type === 'item.started') {
          ...
          // item.completed — replace with final text
        ...
        // file_change has changes[] array with {path, kind}
    ...
    // Use both 'close' and 'exit' — whichever fires first resolves the promise
  ...
  try {
    for (let attempt = 0; attempt <= MAX_TIMEOUT_RETRIES; attempt++) {
      ...
      if (isTimeout && attempt < MAX_TIMEOUT_RETRIES) {
        ...
        currentImages = []; // images already processed in first round
        // New progress message for the next round
        progressMsg = await bot.sendMessage(chatId, `Continuing... (retry ${attempt + 1}/${MAX_TIMEOUT_RETRIES})`);
        continue;
      }

      // Normal completion (or final timeout) — show result
      ...
      // Auto-send image/binary output files
